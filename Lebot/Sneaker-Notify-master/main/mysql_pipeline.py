...
class MYSQL_Pipeline(object):
  ...
  def __init__(self):
    ...
    self.conn = MySQLdb.connect(host='127.0.0.1', user='root', passwd='admin', db='crawler', charset="utf8", use_unicode=True)
    self.conn.ping(True)
  ...
  def process_item(self, item, spider):
    try:
        ...
        if ('kobe' in item['name'].encode('utf-8').lower()) or ('cp9654' in item['name'].encode('utf-8').lower()) or ('purecontrol' in item['name'].encode('utf-8').lower()) or ('yeezy' in item['name'].encode('utf-8').lower()) or ('tom' in item['name'].encode('utf-8').lower() and 'sachs' in item['name'].encode('utf-8').lower()) or ('max' in item['name'].encode('utf-8').lower() and 'master' in item['name'].encode('utf-8').lower()) or ('vandal' in item['name'].encode('utf-8').lower()) or ('ronnie' in item['name'].encode('utf-8').lower()) or ('fieg' in item['name'].encode('utf-8').lower()) or ('flamingo' in item['name'].encode('utf-8').lower()) or ('mars' in item['name'].encode('utf-8').lower() and 'yard' in item['name'].encode('utf-8').lower()):
          ...
          CONSUMER_KEY = 'mx1ilzs7LkppEYMNyAu7XevMu'
          CONSUMER_SECRET = 'HRq9UDNHO6T1C3EQqd5ondQMGw8TSPCHd0qIfK88lW2zBKw0rr'
          ACCESS_TOKEN_KEY = '1613573064-K8H4oA67S3PZyQcXUbqVV1bD73kfCkk3lLSIRC9'
          ACCESS_TOKEN_SECRET = 'KY9eo66vCTXnpfXYN7W556JD1cRAglLPg3SPDmA6y5Fcl'
